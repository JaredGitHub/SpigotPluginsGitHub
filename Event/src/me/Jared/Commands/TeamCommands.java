...
public class TeamCommands implements CommandExecutor, TabCompleter
{
	...
	{
		p.sendMessage(ChatColor.WHITE + "------------" + ChatColor.GOLD + "Team commands" + ChatColor.WHITE + "------------");
	...
	public void teamInfo(Player sender, String teamName)
	{
		...
		sender.sendMessage(ChatColor.GOLD + "Leader: " + ChatColor.RESET + config.getString("team." + teamName + ".Leader"));

		if(config.getBoolean("team." + teamName + ".FriendlyFire") == true)
		{
			sender.sendMessage(ChatColor.GOLD + "Friendly Fire: " + ChatColor.RED + config.getString("team." + teamName + ".FriendlyFire"));
		}else
		{
			sender.sendMessage(ChatColor.GOLD + "Friendly Fire: " + ChatColor.GREEN + config.getString("team." + teamName + ".FriendlyFire"));
		}
	...
	public void teamInfoPassword(Player sender, String teamMember)
	{
		...
		sender.sendMessage(ChatColor.GOLD + "Team: " + ChatColor.RESET + teamName);
		sender.sendMessage(ChatColor.GOLD + "Leader: " + ChatColor.RESET + config.getString("team." + teamName + ".Leader"));
		sender.sendMessage(ChatColor.GOLD + "Password: " + ChatColor.RESET + config.getString("team." + teamName + ".Password"));
		...
		{	
			...
			if(args.length >= 3 && args[0].equalsIgnoreCase("create"))
			{
				...
				try
				{
					...
					else
					{
						...
						ArrayList<String> configList = new ArrayList<String>(config.getStringList("team." + name + ".Members"));
			...
			}else if (args.length == 2 && args[0].equalsIgnoreCase("invite"))
			{
				...
				try
				{
					...
					if(args[1] != null)
					{
						...
						message.setClickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND, "/team join " + teamName + " " + config.getString("team." + teamName + ".Password")));
			...
			}else if(args[0].equals("ff") || (args[0].equals("friendlyfire")))
			{
				try
				{
					...
					if(config.getString("team." + teamName + ".Leader").equals(p.getName()))
					{
						...
						{
							config.set("team." + teamName + ".FriendlyFire", true);
							plugin.saveConfig();
							plugin.reloadConfig();
						...
						if(friendlyFire == true)
						{
							...
							for(String playersinteam : config.getStringList("team." + teamName + ".Members"))
							{
								...
								if(player == p)
								{
									p.sendMessage("You have set friendly fire to " + ChatColor.GREEN + config.getString("team." + teamName + ".FriendlyFire"));
								}else
								{
									player.sendMessage(p.getName() + " has set friendly fire to " + ChatColor.GREEN + config.getString("team." + teamName + ".FriendlyFire"));
								}
							}
							return true;
						}
			...
			{		
				...
				for(String str : config.getStringList("teams"))
				{
					...
					{
						hoverMessage.append("\n" + ChatColor.GOLD + "Friendly Fire: " + ChatColor.RED + config.getString("team." + str + ".FriendlyFire"));
					}else
					{
						hoverMessage.append("\n" + ChatColor.GOLD + "Friendly Fire: " + ChatColor.GREEN + config.getString("team." + str + ".FriendlyFire"));
					}
			...
			}else if (args.length == 2 && args[0].equalsIgnoreCase("promote"))
			{
				try
				{
					...
					String teamName = config.getString("players." + p.getName() + ".team");
					ArrayList<String> members = (ArrayList<String>) config.getStringList("team." + teamName + ".Members");
