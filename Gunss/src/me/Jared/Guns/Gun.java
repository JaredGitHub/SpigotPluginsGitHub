...
public class Gun
{
	...
	public void shoot()
	{
		if(this.owner != null && this.owner.getPlayer().isOnline() && !this.reloading)
		{
			...
			{
				if(this.owner.checkAmmo(this, event.getAmountAmmoNeeded()) && event.getAmountAmmoNeeded() > 0
						|| event.getAmountAmmoNeeded() == 0)
				{
					...
					while(i < this.bulletsPerClick)
					{
						...
						double xwep = ((double) (GunsPlugin.random.nextInt(acc) - GunsPlugin.random.nextInt(acc)) + 0.5)
								/ 1000.0;
						double ywep = ((double) (GunsPlugin.random.nextInt(acc) - GunsPlugin.random.nextInt(acc)) + 0.5)
								/ 1000.0;
						double zwep = ((double) (GunsPlugin.random.nextInt(acc) - GunsPlugin.random.nextInt(acc)) + 0.5)
								/ 1000.0;
